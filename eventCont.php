...
class eventCont extends Controller
{
      public function __construct()
    {
     ...
     public function viewDetailEvent($event_id){
        
        $event = DB::table('event')->where('event_id','=',$event_id)->first();
        $kateven = DB::table('kategori_event')->where('kateven_id','=',$event->kateven_id)->first();
        $wisata = DB::table('wisata')->where('wisata_id','=',$event->wisata_id)->first();
    
        return view('event')
        ->with('event',$event)
        ->with('kateven',$kateven)
        ->with('wisata',$wisata)
        ->with('act','viewDetailEvent');
    }
